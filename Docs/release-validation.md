...
- HLT-178 将抽屉收敛为中性单色界面：聊天历史独立滚动，日程与设置使用紧凑行，底部
  固定 `New chat` 与唯一 `Account` 入口；移除渐变、彩色导航卡和冗长副标题。
- 现有聊天打开、会话操作、日程、设置与账户路由保持不变；底部两项在长聊天历史下无需
  滚动即可访问，并保留 VoiceOver 标识、选中状态和 44 点以上点击区域。
- iPhone 16e（iOS 26.2）模拟器 Debug build 通过，专项 UI 测试实际验证键盘收起、抽屉
  打开、底部两项可点击、顺序正确且 Account 只有一个；1 项测试通过。最终界面已人工
  截图检查。
...
- HLT-180 参考 ChatGPT 移动端层级重做聊天主页：顶部悬浮模型胶囊与左右圆形入口、底部三条
  简洁健康建议，以及包含操作菜单、文本输入、系统听写和发送/停止状态的悬浮输入框。语音
  与麦克风权限从页面启动请求改为用户点击听写后按需请求。
- iPhone 16e（iOS 26.2）模拟器专项 UI 测试 1 项通过，覆盖顶部模型切换、设置、操作菜单、
  听写/波形入口、键盘收起和覆盖式抽屉；最终空白聊天界面已与参考截图并排检查。
