...
| 同意与数据生命周期 | 最小授权 grant/revoke 已在生产验证，正文生命周期待验收 | OMR-017～023 已部署；H54 从已登录 iOS 模拟器产生 `omerChatText` 授权并由 Omer 回查确认，撤销后的空授权也被确认，最后恢复用户选择的最小范围。尚需验证撤回后健康正文停止新增与删除行为 |
